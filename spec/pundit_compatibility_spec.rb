...
describe 'Pundit Compatibility' do
  let(:user) { double }
  ...
  describe ".authorize" do
    it "infers the policy and authorizes based on it" do
      expect(Regulator.authorize(user, post, :update?)).to be_truthy
    end

    it "works with anonymous class policies" do
      expect(Regulator.authorize(user, article_tag, :show?)).to be_truthy
      expect { Regulator.authorize(user, article_tag, :destroy?) }.to raise_error(Regulator::NotAuthorizedError)
    end

    it "raises an error with a query and action" do
      expect { Regulator.authorize(user, post, :destroy?) }.to raise_error(Regulator::NotAuthorizedError, "not allowed to destroy? this #<Post>") do |error|
        expect(error.query).to eq :destroy?
        expect(error.record).to eq post
        expect(error.policy).to eq Regulator.policy(user, post)
      end
    end
  end
  ...
  describe ".policy_scope" do
    it "returns an instantiated policy scope given a plain model class" do
      expect(Regulator.policy_scope(user, Post)).to eq :published
    end

    it "returns an instantiated policy scope given an active model class" do
      expect(Regulator.policy_scope(user, Comment)).to eq Comment
    end

    it "returns an instantiated policy scope given an active record relation" do
      expect(Regulator.policy_scope(user, comments_relation)).to eq comments_relation
    end

    it "returns an instantiated policy scope given an empty active record relation" do
      expect(Regulator.policy_scope(user, empty_comments_relation)).to eq empty_comments_relation
    end

    it "returns nil if the given policy scope can't be found" do
      expect(Regulator.policy_scope(user, Article)).to be_nil
    end

    it "returns nil if blank object given" do
      expect(Regulator.policy_scope(user, nil)).to be_nil
    end
  end
  ...
  describe ".policy_scope!" do
    ...
    it "throws an exception if the given policy scope can't be found" do
      expect { Regulator.policy_scope!(user, Article) }.to raise_error(Regulator::NotDefinedError)
    end

    it "throws an exception if the given policy scope can't be found" do
      expect { Regulator.policy_scope!(user, ArticleTag) }.to raise_error(Regulator::NotDefinedError)
    end

    it "throws an exception if the given policy scope is nil" do
      expect { Regulator.policy_scope!(user, nil) }.to raise_error(Regulator::NotDefinedError, "unable to find policy scope of nil")
    end
  end

  describe ".policy" do
    it "returns an instantiated policy given a plain model instance" do
      policy = Regulator.policy(user, post)
      expect(policy.user).to eq user
      expect(policy.post).to eq post
    end

    it "returns an instantiated policy given an active model instance" do
      policy = Regulator.policy(user, comment)
      expect(policy.user).to eq user
    ...
    it "returns nil if the given policy can't be found" do
      expect(Regulator.policy(user, article)).to be_nil
      expect(Regulator.policy(user, Article)).to be_nil
    end

    it "returns nil if the given policy is nil" do
      expect(Regulator.policy(user, nil)).to be_nil
    end

    describe "with .policy_class set on the model" do
      it "returns an instantiated policy given a plain model instance" do
        policy = Regulator.policy(user, artificial_blog)
        expect(policy.user).to eq user
        expect(policy.blog).to eq artificial_blog
      end

      it "returns an instantiated policy given a plain model class" do
        policy = Regulator.policy(user, ArtificialBlog)
        expect(policy.user).to eq user
      ...
      it "returns an instantiated policy given a plain model instance providing an anonymous class" do
        policy = Regulator.policy(user, article_tag)
        expect(policy.user).to eq user
      ...
      it "returns an instantiated policy given an array" do
        policy = Regulator.policy(user, [:project, :criteria])
  ...
  describe ".policy!" do
    ...
    it "throws an exception if the given policy can't be found" do
      expect { Regulator.policy!(user, article) }.to raise_error(Regulator::NotDefinedError)
      expect { Regulator.policy!(user, Article) }.to raise_error(Regulator::NotDefinedError)
    end

    it "throws an exception if the given policy is nil" do
      expect { Regulator.policy!(user, nil) }.to raise_error(Regulator::NotDefinedError, "unable to find policy of nil")
    end
  end
  ...
  describe "#authorize" do
    ...
    it "can be given a different permission to check" do
      ...
      expect { controller.authorize(post, :destroy?) }.to raise_error(Regulator::NotAuthorizedError)
    end

    it "works with anonymous class policies" do
      expect(controller.authorize(article_tag, :show?)).to be_truthy
      expect { controller.authorize(article_tag, :destroy?) }.to raise_error(Regulator::NotAuthorizedError)
    end

    it "throws an exception when the permission check fails" do
      expect { controller.authorize(Post.new) }.to raise_error(Regulator::NotAuthorizedError)
    end
  ...
  describe "#policy" do
    ...
    it "throws an exception if the given policy can't be found" do
      expect { controller.policy(article) }.to raise_error(Regulator::NotDefinedError)
    end
  ...
  describe "Regulator::NotAuthorizedError" do
    it "can be initialized with a string as message" do
      error = Regulator::NotAuthorizedError.new("must be logged in")
